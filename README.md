...
* **Golden Image Pipeline**: Automate the build of the hardened AlmaLinux CIS templates (VM `1000`/`1001`, and the AlmaLinux 10 image at VM `1002`) themselves, so all hosts stay in sync without a manual template rebuild - the same gap `vault-bootstrap` has open for its own template.
